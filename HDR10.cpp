...
int main(int argc, char* argv[]) {
    ...
    for(byte i = 0; i < bval; i++) {
        ...
        if(res == MONITOR_SUCCESS) {
            res = SetColorSpaceState(COLOR_SPACE2_HDR_PQ);
            if(res == MONITOR_SUCCESS) {
                std::println("Color space has been set to HDR10!");
            }
            else {
    ...
    for(std::wstring dk : device_keys) {
        BOOL flag = WcsSetDefaultColorProfile(
            ...
            CPST_PERCEPTUAL,
            0,
            L"DELL UP3221Q_JZQDR43_HDR10_V4.icm"
        );
        if(flag) {
            std::println("The color profile has been switched to HDR10!");
        }
        else {
