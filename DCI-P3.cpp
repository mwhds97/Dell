...
int main(int argc, char* argv[]) {
    ...
    for(byte i = 0; i < bval; i++) {
        ...
        if(res == MONITOR_SUCCESS) {
            res = SetColorSpaceState(COLOR_SPACE2_CAL_1);
            if(res == MONITOR_SUCCESS) {
                std::println("Color space has been set to DCI-P3!");
            }
            else {
    ...
    for(std::wstring dk : device_keys) {
        BOOL flag = WcsSetDefaultColorProfile(
            ...
            CPST_PERCEPTUAL,
            0,
            L"DELL UP3221Q_JZQDR43_CAL_1_V2.icm"
        );
        if(flag) {
            std::println("The color profile has been switched to DCI-P3!");
        }
        else {
