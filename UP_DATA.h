...
typedef enum color_preset
{
	...
	COLOR_PRESET_GAME_2       = 0x00010000, /* 3.2.0 */
	COLOR_PRESET_GAME_3       = 0x00020000, /* 3.2.0 */
	COLOR_PRESET_TEXT         = 0x00040000, /* 3.2.0 */
	COLOR_PRESET_SPORTS       = 0x00080000, /* 3.2.0 */
	COLOR_PRESET_MULTI_MATCH  = 0x00100000  /* 3.2.0 */
}
COLOR_PRESET;

typedef enum hdr_color_preset
{
	HDR_COLOR_PRESET_OFF     = 0x00000001, /* 3.2.0 */
	HDR_COLOR_PRESET_MOVIE   = 0x00000002, /* 3.2.0 */
	HDR_COLOR_PRESET_GAME    = 0x00000004, /* 3.2.0 */
	HDR_COLOR_PRESET_VIVID   = 0x00000008, /* 3.2.0 */
	HDR_COLOR_PRESET_DESKTOP = 0x00000010, /* 3.2.0 */
	HDR_COLOR_PRESET_DISPLAY = 0x00000020  /* 3.2.0 */
}
HDR_COLOR_PRESET;
...
typedef enum uniformity_compensation
{
	...
	UNIFORMITY_COMPENSATION_CALIBRATED_LOW  = 1, /* 3.2.0 */
...
typedef enum video_input
{
	...
	VIDEO_INPUT_HDMI4 = 0x0001000, /* 3.2.0 */
	VIDEO_INPUT_MDP1  = 0x0002000, /* 3.2.0 */
	VIDEO_INPUT_MDP2  = 0x0004000, /* 3.2.0 */
	VIDEO_INPUT_MDP3  = 0x0008000, /* 3.2.0 */
	VIDEO_INPUT_USBC1 = 0x0010000, /* 3.2.0 */
	VIDEO_INPUT_USBC2 = 0x0020000, /* 3.2.0 */
	VIDEO_INPUT_USBC3 = 0x0040000, /* 3.2.0 */
	VIDEO_INPUT_USBC4 = 0x0080000  /* 3.2.0 */
}
VIDEO_INPUT;
...
typedef enum usb_assoc_type
{
	USB_ASSOC_UNKNOWN   = 0x00,
	USB_ASSOC_VIDEO_USB = 0x01,
	USB_ASSOC_SELECTSW  = 0x02
}
USB_ASSOC_TYPE;

typedef enum usb_port
{
	USB_PORT_B1 = 0x01,
	USB_PORT_B2 = 0x02,
	USB_PORT_C1 = 0x03,
...
typedef enum pxp_layout
{
	PXP_LAYOUT_NONE                = 0x00, //Not in PIP or PBP mode
	PXP_LAYOUT_PIP_SMALL           = 0x01, //PIP: Small
	PXP_LAYOUT_PIP_LARGE           = 0x02, //PIP: Large
	PXP_LAYOUT_2W_HORIZONTAL_SPLIT = 0x10, //PBP: 2 ratios top and bottom
	PXP_LAYOUT_3W_RIGHT_SPLIT      = 0x11, //PBP: 2 ratios for top and bottom on the right panel (fixed at 50% now)
	PXP_LAYOUT_3W_LEFT_SPLIT       = 0x12, //PBP: 2 ratios for top and bottom on the left panel (fixed at 50% now)
	PXP_LAYOUT_3W_BOTTOM_SPLIT     = 0x13, //PBP: 2 ratios for left and right on the bottom panel (fixed at 50% now)
	PXP_LAYOUT_3W_TOP_SPLIT        = 0x14, //PBP: 2 ratios for left and right on the top panel (fixed at 50% now)
	PXP_LAYOUT_3W_SIDE_BY_SIDE     = 0x15, //PBP: 2 ratios for left, center (fixed at equal sizes now), right will be remaining ratio left
	PXP_LAYOUT_4W_QUAD             = 0x16  //PBP: 2 ratios for vertical and horizontal divisors
}
PXP_LAYOUT;

typedef enum pxp_pip_size
{
	PXP_PIP_SIZE_SMALL = 0x01,
	PXP_PIP_SIZE_LARGE = 0x02
}
PXP_PIP_SIZE;
...
typedef struct PXPLayoutStruct {
	BYTE LayoutType;        //enum PXP_LAYOUT
	BYTE Ratio[2];   
	UWORD32 VideoInput[4];  //enum VIDEO_INPUT
	BYTE USBAssociation[4]; //enum USB_PORT	
}
PXPLayoutStructType; //Total bytes = 3 + 4*4 + 4 = 23
